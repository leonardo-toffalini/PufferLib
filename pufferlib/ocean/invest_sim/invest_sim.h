...
void c_render(InvestSim* env) {
    ...
    // Draw axes
    DrawLine(margin, screen_height - margin, screen_width - margin, screen_height - margin, BLACK);
    DrawLine(margin, margin, margin, screen_height - margin, BLACK);
    ...
    for (int i = 0; i <= env->tick; i++) {
        ...
        max_assets = fmaxf(max_assets, fmaxf(env->riskless_asset, env->risky_asset));
    }
    ...
    for (int i = 1; i <= env->tick; i++) {
        ...
        float y1 = screen_height - margin - (env->prices[i - 1] / max_value) * graph_height;
        float x2 = margin + i * graph_width / (float)env->time_horizon;
        float y2 = screen_height - margin - (env->prices[i] / max_value) * graph_height;
        DrawLine(x1, y1, x2, y2, RED);
    }

    // Draw riskless asset line (green)
    for (int i = 1; i <= env->tick; i++) {
        float x1 = margin + (i - 1) * graph_width / (float)env->time_horizon;
        float y1 = screen_height - margin - (env->riskless_asset / max_value) * graph_height;
        float x2 = margin + i * graph_width / (float)env->time_horizon;
        float y2 = screen_height - margin - (env->riskless_asset / max_value) * graph_height;
        DrawLine(x1, y1, x2, y2, GREEN);
    }

    // Draw risky asset line (blue)
    for (int i = 1; i <= env->tick; i++) {
        float x1 = margin + (i - 1) * graph_width / (float)env->time_horizon;
        float y1 = screen_height - margin - (env->risky_asset / max_value) * graph_height;
        float x2 = margin + i * graph_width / (float)env->time_horizon;
        float y2 = screen_height - margin - (env->risky_asset / max_value) * graph_height;
        DrawLine(x1, y1, x2, y2, BLUE);
    }

    // Draw labels
    DrawText("Price", margin - 40, margin - 20, 20, RED);
    DrawText("Riskless Asset", margin - 40, margin, 20, GREEN);
    DrawText("Risky Asset", margin - 40, margin + 20, 20, BLUE);

    // Draw current values
    char value_text[100];
    sprintf(value_text, "Price: %.2f", env->prices[env->tick]);
    DrawText(value_text, screen_width - 200, margin - 20, 20, RED);
    sprintf(value_text, "Riskless: %.2f", env->riskless_asset);
    DrawText(value_text, screen_width - 200, margin, 20, GREEN);
    sprintf(value_text, "Risky: %.2f", env->risky_asset);
    DrawText(value_text, screen_width - 200, margin + 20, 20, BLUE);
