...
// Helper function to check if a move leads to immediate capture opportunity for opponent
int move_leads_to_capture(Checkers *env, Move m) {
  ...
  if (move_size(m) == 2) {
    Position between_pos = (Position){(m.from.r + m.to.r) / 2, (m.from.c + m.to.c) / 2};
    env->observations[p2i(env, between_pos)] = EMPTY;
  }
  ...
  if (move_size(m) == 2) {
    Position between_pos = (Position){(m.from.r + m.to.r) / 2, (m.from.c + m.to.c) / 2};
    int captured_piece = env->current_player == AGENT ? OPPONENT_PAWN : AGENT_PAWN;
...
float evaluate_move(Checkers *env, Move m) {
  ...
  int center_distance_from = abs(m.from.r - env->size/2) + abs(m.from.c - env->size/2);
  int center_distance_to = abs(m.to.r - env->size/2) + abs(m.to.c - env->size/2);
  ...
  if (piece == AGENT_PAWN || piece == AGENT_KING) {
    ...
    for (int d = 0; d < 4; d++) {
      ...
      if (protect_r >= 0 && protect_r < env->size && protect_c >= 0 && protect_c < env->size) {
        int protected_piece = env->observations[protect_r * env->size + protect_c];
...
void scripted_strong_move(Checkers *env) {
  ...
  int move_type = -1;
  if (dr == -1 && dc == -1) move_type = 0;
  else if (dr == -1 && dc == 1) move_type = 1;
  else if (dr == 1 && dc == -1) move_type = 2;
  else if (dr == 1 && dc == 1) move_type = 3;
  else if (dr == -2 && dc == -2) move_type = 4;
  else if (dr == -2 && dc == 2) move_type = 5;
  else if (dr == 2 && dc == -2) move_type = 6;
  else if (dr == 2 && dc == 2) move_type = 7;
...
void scripted_expert_move(Checkers *env) {
  ...
  for (int i = 0; i < moves.count; i++) {
    ...
    if (move_size(moves.moves[i]) == 2) {
      Position between_pos = (Position){(moves.moves[i].from.r + moves.moves[i].to.r) / 2, 
    ...
    if (move_size(moves.moves[i]) == 2) {
      Position between_pos = (Position){(moves.moves[i].from.r + moves.moves[i].to.r) / 2, 
                                       (moves.moves[i].from.c + moves.moves[i].to.c) / 2};
      int captured_piece = current_player_backup == AGENT ? OPPONENT_PAWN : AGENT_PAWN;
      env->observations[p2i(env, between_pos)] = captured_piece;
    }
    ...
    float move_score = evaluate_move(env, moves.moves[i]) - opponent_best_score * 0.5f;
...
void scripted_step(Checkers *env, int difficulty) {
  ...
  case 1:
    ...
    break;
  case 2:
    scripted_strong_move(env);
    break;
  case 3:
    scripted_expert_move(env);
    break;
  default:
    scripted_expert_move(env);
    break;
...
void c_step(Checkers *env) {
  ...
  scripted_step(env, env->difficulty);
  if (env->terminals[0] == 1) {
    add_log(env);
    c_reset(env);
    return;
